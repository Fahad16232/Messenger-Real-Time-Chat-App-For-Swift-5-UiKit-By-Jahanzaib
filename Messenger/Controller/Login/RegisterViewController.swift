...
final class RegisterViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        firstNameField.frame = CGRect(x: 30, y: imageView.bottom+10, width: scrollView.width-60, height: 42)
        lastNameField.frame = CGRect(x: 30, y: firstNameField.bottom+10, width: scrollView.width-60, height: 42)
        emailField.frame = CGRect(x: 30, y: lastNameField.bottom+10, width: scrollView.width-60, height: 42)
        passwordField.frame = CGRect(x: 30, y: emailField.bottom+10, width: scrollView.width-60, height: 42)
        registerButton.frame = CGRect(x: 30, y: passwordField.bottom+10, width: scrollView.width-60, height: 42)
    }
    private func switchFirstResponder() {
        if firstNameField.isFirstResponder {
            ...
            // Perform any additional actions or validation when the last field is completed
        }
    }
...
//MARK:   //FireBase Login
        ...
        DatabaseManager.shared.userExists(with: email, completion: { [weak self] exists in
            ...
            guard !exists else {
                // User already exists
                strongSelf.alertUserLoginError(message: "User account for that email is already exist!")
                return
    ...
    func alertUserLoginError(message : String = "Please enter all information to create a new account"){
        let alert = UIAlertController(title: "Oopss", message: message, preferredStyle: .alert)
...
//MARK: Move Curosr from one text field to other
...
extension RegisterViewController: UITextFieldDelegate {
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        case emailField:
            passwordField.becomeFirstResponder()
        case passwordField:
            passwordField.resignFirstResponder()  // Dismiss keyboard on pressing return
            registerButtonTapped()
        default:
            break
        }
